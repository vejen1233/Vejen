...
export default function vejen() {
  ...
  function updateQty(id, qty) { const q = Math.max(0, Math.min(99, Number(qty) || 0)); setCart(c => ({ ...c, [id]: q })); }
  function clearCart() { setCart({}); }

  function checkout() {
    if (total <= 0) return;
    const orderLines = items.filter(it => it.qty > 0)
              ...
              {PRODUCTS.map((p) => (
                <article key={p.id} className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="mt-5 flex items-center gap-3">
                    <button onClick={() => addToCart(p.id)} className="px-4 py-2 rounded-xl bg-neutral-900 text-white text-sm font-medium hover:opacity-90">Læg i kurv</button>
